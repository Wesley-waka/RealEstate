...
export const FeaturedCard = ({ item: { rating, image, name, address, price }, onPress }: Props) => {
  return (
    <TouchableOpacity onPress={onPress} className='flex flex-col items-start w-60 h-80 relative'>
      <Image source={{ uri: image }} className='size-full rounded-2xl' />
      <Image source={images.cardGradient} className='size-full rounded-2xl absolute bottom-0' />
      <View className='flex-row items-center bg-white/90 px-3 py-1.5 rouned-full absolute top-5 right-5'>
      ...
      <View className='flex flex-col item-start absolute bottom-5 inset-x-5'>
        ...
        <Text className='text-base font-0rubik text-white'>
          {address}
        </Text>
